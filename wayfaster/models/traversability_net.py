...
class TravNet(nn.Module):
    ...
    def __init__(
        ...
        self.camC = image_dim  # C in the paper (LIFT's context feature vector dimension) is configured by LATENT_DIM = 64 (set in temporal_model.yaml)
        ...
        self.temporal_model = TemporalModel(
            channels=self.camC, temporal_length=temporal_length, input_shape=(nx[0], nx[1])
        )
    ...
    def sample2bev(self, geometry, x):
        ...
        """
        # I think Z is Depth, X is u, and Y is v, since geom is the frustrum.
        ...
        depth = (
            2
            * (geometry[..., 2] - self.grid_bounds["dbound"][0])
    ...
    def forward(self, color_img, pcloud, intrinsics, extrinsics, depth_img=None):
        ...
        """
        # B is batch_size, T or N is temporal_length (#imgs in seq).
        ...
        # x is the outer product: (B*N, 1, D, H/downsample, W/downsample) elementwise multiply (B*N, C, 1, H/downsample, W/downsample)
        # => (B*N, C, D, H/downsample, W/downsample)
        x = depth_logits.softmax(dim=1).unsqueeze(1) * depth_context.unsqueeze(2)

        # Create 3D frustrum (from predefined voxels (x,y,z), think of it as empty 3doccupancygrid map).
        # Frustrum coordinate system is (u,v,depth along that ray), instead of (x,y,z), and it's stored in `geom`
        ...
        # And compress X from B*T x C*Z x X x Y to B*T x C x X x Y
